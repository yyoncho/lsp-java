...
(defun lsp-java-spring-initializr ()
  ...
  (interactive)
  (flet ((ask (message key) (alist-get 'id (lsp--completing-read message
        ...
        (-let* ((group-id (read-string "Enter group name: " "com.example"))
          ...
          (while (setq dep (-> (lsp--completing-read
            ...
            (if (-contains? deps dep)
              ...
              (pushnew dep deps)))
          (message (format "%sstarter.zip?type=%s&language=%s&groupId=%s&artifactId=%s&packaging=%s&bootVersion=%s&baseDir=%s&dependencies=%s"
                           base-url type language group-id artifact-id packaging boot-version artifact-id (s-join "," deps)))
          (url-copy-file
           (format "%sstarter.zip?type=%s&language=%s&groupId=%s&artifactId=%s&packaging=%s&bootVersion=%s&baseDir=%s&dependencies=%s"
                   base-url type language group-id artifact-id packaging boot-version artifact-id (s-join "," deps))
           temp-file
           t)
